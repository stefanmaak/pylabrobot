...
async def check_communications(link: Link) -> FirmwareVersion:
  ...
  Two frames, bracketed as one operation: the liveness check, then the firmware version record.
  ...
  async with link.operation(Operation("ping")):
    ...
    answer = await link.request(command, operation="ping")
  try:
    version = command.parse(answer)
